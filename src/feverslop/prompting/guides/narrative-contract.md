...
"milestone_bindings" (required when "milestone_order" is non-empty: exactly one object per ordered milestone, with {"milestone_id", "location_id", "relative_position"}; use each milestone id exactly once, only canonical location ids, and a relative_position from 0.0 to 1.0 within that location's phase),
