...
def _next_valid_17n5(n: int) -> int:
    """Round *n* up to the next 17N+5 value.

    For n <= 0 returns 5.  For n >= 1 returns the smallest m >= n such that
    m % 17 == 5.
    """
    if n <= 0:
        return 5
...
def _frames_from_duration(seconds: float) -> int:
    ...
    return _next_valid_17n5(raw_frames)


def _duration_from_frames(frames: int) -> float:
    """Convert a 17N+5-constrained frame count to seconds at 24 fps."""
    return frames / 24.0
