...
class StartframeEngineTests(unittest.TestCase):
    ...
    def test_director_prompts_fail_clearly_when_startframe_plan_is_missing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(ValueError, "Missing required startframe plan"):
    ...
    def test_director_prompts_fail_clearly_when_identity_ledger_is_missing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            build_startframe_identity_ledger(project_dir=project)
            build_startframe_plan(project_dir=project)
            (project / "movie" / "identity_ledger.json").unlink()

            with self.assertRaisesRegex(ValueError, "Missing required identity ledger"):
                build_startframe_director_prompts(project_dir=project)
