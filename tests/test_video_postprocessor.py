...
class VideoPostProcessorConcatTests(unittest.TestCase):
    ...
    def test_trim_clip_pads_short_output_to_requested_frames(self):
        ...
        with (
            ...
            # Third call is the padding step (tpad + frames:v)
    ...
    def test_trim_clip_reencode_uses_trim_filters(self):
        """Verify the reencode path uses -vf trim rather than input -t / -frames:v.

        This prevents GOP tail artifacts: the trim filter selects frames by
        index (deterministic), while input -t depends on PTS/GOP alignment.
        Omitting -frames:v avoids encoder GOP-pad duplication, and -vsync 0
        prevents muxer insertion of duplicate frames.
        """
        processor = VideoPostProcessor(ffmpeg_path="ffmpeg", reencode=True)
        spec = TrimSpec(
            source_file=Path("raw.mp4"),
            output_file=Path("scene_0001.mp4"),
            fps=24,
        ...
        with patch("feverslop.adapters.video_postprocessor.subprocess.run") as run:
            ...
            # No -ss when start_seconds == 0
            self.assertNotIn("-ss", cmd)

    def test_trim_clip_reencode_uses_ss_when_offset_positive(self):
        """Verify that -ss is placed before -i when trim_front_frames > 0."""
        processor = VideoPostProcessor(ffmpeg_path="ffmpeg", reencode=True)
        spec = TrimSpec(
            source_file=Path("raw.mp4"),
            output_file=Path("scene_0001.mp4"),
