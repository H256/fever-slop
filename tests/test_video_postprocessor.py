...
class VideoPostProcessorConcatTests(unittest.TestCase):
    def test_concat_clips_preserves_scene_audio_by_default(self):
    ...
    def test_diagnostic_original_audio_mux_is_explicit(self):
        processor = VideoPostProcessor(ffmpeg_path="ffmpeg")

        with patch("video_postprocessor.subprocess.run") as run:
            output = processor.mux_original_audio_for_diagnostics(
                video_file=Path("final_concat_video_only.mp4"),
                audio_file=Path("song.mp3"),
                output_file=Path("final_concat_original_audio_debug.mp4"),
            )

        cmd = run.call_args.args[0]
        self.assertEqual(Path("final_concat_original_audio_debug.mp4"), output)
