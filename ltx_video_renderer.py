...
class LTXVideoRenderer:
    """
    Renders LTX I2V clips from render_plan.json.

    Expected ComfyUI workflow _meta.title anchors:
    #WIDTH, #HEIGHT, #LOAD_AUDIO, #TRIM_AUDIO, #STARTFRAME,
    #FRAMES, #FRAMERATE, #SEED, #PROMPT_RELAY, #SAVE_VIDEO
    """
    ...
    def render_videos(
        ...
        for scene in render_plan:
            ...
            output_path = self.output_dir / f"scene_{scene_number:04}.mp4"
            ...
            if not self.allow_out_of_range_clips:
                if duration < self.min_duration or duration > self.max_duration:
                    raise ValueError(
                        f"Scene {scene_number} duration {duration:.3f}s is outside "
                        f"configured range {self.min_duration:.3f}s..{self.max_duration:.3f}s. "
                        f"Repair scene SRT/render plan before rendering."
                    )
            ...
            if not startframe_path.exists():
                raise FileNotFoundError(
                    f"Missing storyboard startframe for scene {scene_number}: {startframe_path}"
                )
            ...
            clip_path = self.render_scene_video(
                scene=scene,
        ...
        self.write_concat_list(rendered_files, self.output_dir / "concat_list.txt")
    ...
    def render_scene_video(
        ...
        start_seconds = float(scene["abs_start_seconds"])

        # Project rule: frame_count = round(fps * duration_seconds) + 1.
        # Therefore the frame-span duration is:
        duration_seconds = (frame_count - 1) / float(fps)

        global_prompt, local_prompts, segment_lengths = self._build_prompt_relay_payload(scene)
        ...
        patcher.set_input_by_title(self.frames_node_title, "value", frame_count)
        ...
        patcher.set_input_by_title(self.trim_audio_node_title, "start_index", start_seconds)
        patcher.set_input_by_title(self.trim_audio_node_title, "duration", duration_seconds)
        ...
        patcher.set_input_by_title(self.save_video_node_title, "filename_prefix", f"ltx/scene_{scene_number:04}")
        ...
        if self.debug_workflows_dir:
            ...
            debug_path = self.debug_workflows_dir / f"scene_{scene_number:04}_workflow.json"
        ...
        first = videos[0]
        output_path = self.output_dir / f"scene_{scene_number:04}_comfyui.mp4"
    ...
    def _build_prompt_relay_payload(self, scene: dict) -> tuple[str, str, str]:
        ltx = scene["ltx"]
        global_prompt = ltx["base_prompt"].strip()
        frame_count = int(scene["frame_count"])
        relays = ltx.get("prompt_relay", [])

        timeline_frames = frame_count if self.segment_length_mode == "frames" else max(1, frame_count - 1)

        if not relays:
            return global_prompt, "", str(timeline_frames)
        ...
        for relay in relays:
            start = max(0, min(int(relay["frame_start"]), timeline_frames))
            end = max(start, min(int(relay["frame_end"]), timeline_frames))
        ...
        if cursor < timeline_frames:
            local_prompts.append("hold the same shot, subtle breathing and atmospheric motion")
            segment_lengths.append(timeline_frames - cursor)
